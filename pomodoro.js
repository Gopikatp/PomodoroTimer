...
function startTimer(duration) {
    ...
    timer = setInterval(() => {
        if (timeLeft <= 0) {
            clearInterval(timer);
            alert(`${currentMode === 'focus' ? "Focus time's up!" : currentMode === 'shortBreak' ? "Short break's over!" : "Long break's over!"}`);
            if (currentMode === 'shortBreak' || currentMode === 'longBreak') {
                document.getElementById("skipBreakButton").style.display = true; // Show skip button
            }
